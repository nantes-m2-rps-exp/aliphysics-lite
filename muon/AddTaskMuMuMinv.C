...
///
/// \author: L. Aphecetche (Subatech) (laurent.aphecetche - at -
/// subatech.in2p3.fr)
///
...
AliAnalysisTask *AddTaskMuMuMinv(const char *foldername,
                                 ...
                                 const char *beamYear, Bool_t simulations) {
  ...
  AliAnalysisMuMuEventCutter *eventCutter = new AliAnalysisMuMuEventCutter(
      triggerClassesToConsider, triggerInputsToUse);
  ...
  //
  AliAnalysisMuMuCutElement *eventTrue =
      cr->AddEventCut(*eventCutter, "IsTrue", "const AliVEvent&", "");

  AliAnalysisMuMuCutElement *ps = eventTrue;

  if (!simulations) {
    cr->AddCutCombination(cr->AddEventCut(*eventCutter, "IsPhysicsSelectedANY",
                                          "const AliInputEventHandler&", ""));
  }
  ...
  AliAnalysisMuMuCutElement *triggerSelection = cr->AddTriggerClassCut(
      *eventCutter, "SelectTriggerClass",
      "const TString&,TString&,UInt_t,UInt_t,UInt_t", "");
  ...
  AliAnalysisMuMuCutElement *trackTrue =
      cr->AddTrackCut(*cr, "AlwaysTrue", "const AliVParticle&", "");

  if (singleAnalysis) {
    AliAnalysisMuMuCutElement *rabs =
        cr->AddTrackCut(*singleAnalysis, "IsRabsOK", "const AliVParticle&", "");
    AliAnalysisMuMuCutElement *eta = cr->AddTrackCut(
        *singleAnalysis, "IsEtaInRange", "const AliVParticle&", "");
    AliAnalysisMuMuCutElement *matchlow = cr->AddTrackCut(
        *singleAnalysis, "IsMatchingTriggerLowPt", "const AliVParticle&", "");
    ...
    if (minvAnalysis) {
      AliAnalysisMuMuCutElement *pairTrue = cr->AddTrackPairCut(
          *cr, "AlwaysTrue", "const AliVParticle&, const AliVParticle&", "");

      AliAnalysisMuMuCutElement *pairy =
          cr->AddTrackPairCut(*minvAnalysis, "IsRapidityInRange",
                              "const AliVParticle&,const AliVParticle&", "");
  ...
  output.Form("mumu.minv.root:%s", foldername);

  // Create containers for input/output
  AliAnalysisDataContainer *cinput = mgr->GetCommonInputContainer();

  AliAnalysisDataContainer *coutputHC = mgr->CreateContainer(
      Form("OC_%s", foldername), AliMergeableCollection::Class(),
      AliAnalysisManager::kOutputContainer, output.Data());

  AliAnalysisDataContainer *coutputCC = mgr->CreateContainer(
      Form("CC_%s", foldername), AliCounterCollection::Class(),
      AliAnalysisManager::kOutputContainer, output.Data());

  AliAnalysisDataContainer *cparam = mgr->CreateContainer(
      Form("BIN_%s", foldername), AliAnalysisMuMuBinning::Class(),
      AliAnalysisManager::kParamContainer, output.Data());
