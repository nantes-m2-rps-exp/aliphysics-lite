...
ClassImp(AliMuonTrackCuts) // Class implementation in ROOT context
    ...
    : AliAnalysisCuts(), fIsMC(kFALSE), fUseCustomParam(kFALSE),
      fSharpPtCut(kFALSE), fAllowDefaultParams(kFALSE), fPassName(""),
      fOADBParam() {
  /// Default ctor.
}
...
AliMuonTrackCuts::AliMuonTrackCuts(const char *name, const char *title)
    : AliAnalysisCuts(name, title), fIsMC(kFALSE), fUseCustomParam(kFALSE),
      fSharpPtCut(kFALSE), fAllowDefaultParams(kFALSE), fPassName(""),
...
AliMuonTrackCuts::AliMuonTrackCuts(const AliMuonTrackCuts &obj)
    : AliAnalysisCuts(obj), fIsMC(obj.fIsMC),
      fUseCustomParam(obj.fUseCustomParam), fSharpPtCut(obj.fSharpPtCut),
      fAllowDefaultParams(obj.fAllowDefaultParams), fPassName(obj.fPassName),
...
//________________________________________________________________________
void AliMuonTrackCuts::SetCustomParamFromRun(Int_t runNumber,
                                             TString passName) {
...
AliOADBMuonTrackCutsParam *AliMuonTrackCuts::CustomParam() {
  /// Returning the muon track cuts parameters (not const, so you can change the
  /// parameters) CAVEAT: if you only want to Get the parameters, please use
  /// GetMuonTrackCutsParam() If you want to modify the parameters, you need to
  /// call SetCustomParamFromRun at least once, otherwise CustomParam returns a
  /// null pointer.

  if (!fUseCustomParam) {
    AliError(
        "This method allows you to modify the paramaters.\nIf you only want to "
        "get them, please use GetMuonTrackCutsParam instead.\nOtherwise, "
        "please call at least once SetCustomParamFromRun.");
    return 0x0;
  }
  return &fOADBParam;
}
...
TString AliMuonTrackCuts::GuessPass(const AliVEventHandler *eventHandler) {
  ...
  if (passName.IsNull()) {
    ...
    //      AliInfo(Form("Getting pass number from prodInfo: pass%i",
    //      passNumber));
    //    }
    //    else {
...
Bool_t AliMuonTrackCuts::ReadParamFromOADB(Int_t runNumber, TString passName) {
  ...
  if (passName.IsNull() && !fAllowDefaultParams)
    AliFatal("Pass name not specified! Please provide one or allow for default "
             "parameters");

  TString filename =
      AliDataFile::GetFileNameOADB("PWG/MUON/MuonTrackCuts.root");
  ...
  if (fIsMC)
    containerName.ReplaceAll("_data", "_MC");
  TString defaultName = "default";

  AliOADBContainer *oadbContainer =
      static_cast<AliOADBContainer *>(file->Get(containerName.Data()));
  if (!oadbContainer) {
    AliFatal(Form("OADB container %s not found in %s!", containerName.Data(),
                  filename.Data()));
    return kFALSE;
  }
  AliOADBMuonTrackCutsParam *param = static_cast<AliOADBMuonTrackCutsParam *>(
      oadbContainer->GetObject(runNumber, defaultName.Data(), passName));

  TString paramName = param->GetName();
  if (paramName == "default") {
    if (!fAllowDefaultParams) {
      AliFatal(Form("Requested run %i not found in %s! Please check your pass "
                    "name or allow default parameters",
                    runNumber, passName.Data()));
    ...
    oadbContainer =
        static_cast<AliOADBContainer *>(file->Get(containerName.Data()));
    param = static_cast<AliOADBMuonTrackCutsParam *>(
        oadbContainer->GetObject(runNumber, defaultName.Data(), passName));

    AliWarning(Form("Requested run %i not found in %s: using %s", runNumber,
                    passName.Data(), param->GetName()));
  }

  fOADBParam = *param;
  file->Close();

  AliInfo(Form("Requested run %i in pass %s. Param. set: %s", runNumber,
               passName.Data(), fOADBParam.GetName()));
...
Bool_t AliMuonTrackCuts::IsSelected(TObject *obj) {
  ...
  AliDebug(1, Form("IsMuon %i  selected %i  mask 0x%x",
                   AliAnalysisMuonUtility::IsMuonTrack(
                       static_cast<AliVParticle *>(obj)),
                   (selectionMask & filterMask) == filterMask, selectionMask));
...
UInt_t AliMuonTrackCuts::GetSelectionMask(const TObject *obj) {
  ...
  if (eta > -4. && eta < -2.5)
    selectionMask |= kMuEta;

  Double_t thetaAbsEndDeg = AliAnalysisMuonUtility::GetThetaAbsDeg(track);

  if (thetaAbsEndDeg > 2. && thetaAbsEndDeg < 10.)
    selectionMask |= kMuThetaAbs;
  ...
  for (Int_t ilevel = 0; ilevel < 3; ilevel++) {
    if (matchTrig < ilevel + 1)
      break;
    if (fSharpPtCut && pt < fOADBParam.GetSharpPtCut(ilevel))
      break;
    selectionMask |= cutLevel[ilevel];
  }

  if (AliAnalysisMuonUtility::GetChi2perNDFtracker(track) <
      fOADBParam.GetChi2NormCut())
    selectionMask |= kMuTrackChiSquare;
  ...
  Double_t sigmaPdca = IsThetaAbs23(track) ? fOADBParam.GetSigmaPdca23()
                                           : fOADBParam.GetSigmaPdca310();
  ...
  // The relative momentum distribution Sigma_p/p is estimated from the track
  // Delta_p, and from the error on sagitta Delta_s: Delta_p/p =
  // p*Delta_s/(1+p*Delta_s) A cut at N sigmas requres a cut in N Delta_s, so
  // Sigma_p/p(N) = p*N*Delta_s/(1+p*N*Delta_s)
  // Double_t pResolutionEffect = pDca * pTot * GetRelPResolution() / ( 1. +
  // GetNSigmaPdca() * GetRelPResolution()*pTot );

  // Double_t pResolutionEffect = 0.4 * pTot;  // Values used in 2010 data
  // Double_t pResolutionEffect = 0.32 * pTot; // Values in 2011
  // Double_t sigmaPdcaWithRes = TMath::Sqrt( sigmaPdca*sigmaPdca +
  // pResolutionEffect*pResolutionEffect );
  ...
  // p_meas x DCA < N * Sigma_pDCA_meas / ( 1 - N*Delta_s*p_meas /
  // (1+n*Delta_s*p_meas)) Finally the cut value has to be summed in quadrature
  // with the error on DCA, which is given by the slope resolution p_meas x DCA
  // < N * Sqrt( ( Sigma_pDCA_meas / ( 1 - N*Delta_s*p_meas /
  // (1+n*Delta_s*p_meas)) )^2 + (distance * sigma_slope * p_meas )^2)
  Double_t nrp =
      fOADBParam.GetNSigmaPdca() * fOADBParam.GetRelPResolution() * pTot;
  Double_t pResolutionEffect = sigmaPdca / (1. - nrp / (1. + nrp));
  Double_t slopeResolutionEffect =
      535. * fOADBParam.GetSlopeResolution() * pTot;

  Double_t sigmaPdcaWithRes =
      TMath::Sqrt(pResolutionEffect * pResolutionEffect +
                  slopeResolutionEffect * slopeResolutionEffect);

  if (pDca < fOADBParam.GetNSigmaPdca() * sigmaPdcaWithRes)
    selectionMask |= kMuPdca;
...
TVector3 AliMuonTrackCuts::GetCorrectedDCA(const AliVParticle *track) const {
  ...
  TVector3 vertex(AliAnalysisMuonUtility::GetXatVertex(track),
                  AliAnalysisMuonUtility::GetYatVertex(track),
                  AliAnalysisMuonUtility::GetZatVertex(track));

  TVector3 dcaTrack(AliAnalysisMuonUtility::GetXatDCA(track),
                    AliAnalysisMuonUtility::GetYatDCA(track),
                    AliAnalysisMuonUtility::GetZatDCA(track));
...
Double_t AliMuonTrackCuts::GetAverageMomentum(const AliVParticle *track) const {
  ...
  Double_t pTotMean = 0.;
  Double_t pTot = track->P();
  // if ( isESD ) pTotMean = 0.5 * ( pTot +
  // ((AliESDMuonTrack*)track)->PUncorrected() );
  if (!AliAnalysisMuonUtility::IsAODTrack(track)) {
    pTotMean =
        ((AliESDMuonTrack *)track)
            ->PUncorrected(); // Increased stability if using uncorrected value
  } else {
    Double_t meanPcorr = IsThetaAbs23(track) ? fOADBParam.GetMeanPCorr23()
                                             : fOADBParam.GetMeanPCorr310();
...
//________________________________________________________________________
Bool_t
AliMuonTrackCuts::TrackPtCutMatchTrigClass(const AliVParticle *track,
                                           const TArrayI ptCutFromClass) const {
  ...
  if (IsApplySharpPtCutInMatching()) {
    matchTrackerPt = (track->Pt() >=
                      fOADBParam.GetSharpPtCut(ptCutFromClass[0] - 1, kFALSE));
  }
  Bool_t passCut = ((matchTrig >= ptCutFromClass[0]) && matchTrackerPt);
  AliDebug(1, Form("Class matchTrig %i %i  trackMatchTrig %i  trackPt %g "
                   "(required %i)  passCut %i",
                   ptCutFromClass[0], ptCutFromClass[1], matchTrig, track->Pt(),
                   IsApplySharpPtCutInMatching(), passCut));
  return passCut;
}
...
void AliMuonTrackCuts::Print(Option_t *option) const {
  ...
  TString sopt(option);
  sopt.ToLower();
  if (sopt.IsNull() || sopt.Contains("*") || sopt.Contains("all"))
    sopt = "mask param";
  UInt_t filterMask = GetFilterMask();
  Int_t cutLevel[3] = {kMuMatchApt, kMuMatchLpt, kMuMatchHpt};
  ...
  if (sopt.Contains("mask")) {
    ...
    printf("  0x%x\n", filterMask);
    if (filterMask & kMuEta)
      printf("  -4 < eta < -2.5\n");
    if (filterMask & kMuThetaAbs)
      printf("  2 < theta_abs < 10 deg\n");
    if (filterMask & kMuPdca)
      printf("  pxDCA cut\n");
    for (Int_t ilevel = 0; ilevel < 3; ilevel++) {
      if (filterMask & cutLevel[ilevel]) {
        printf("  match %s", cutLevelName[ilevel].Data());
        if (fSharpPtCut)
          printf(" && sharp pt from tracker");
        printf("\n");
      }
    }
    if (filterMask & kMuTrackChiSquare)
      printf("  Chi2 cut on track\n");
    printf(" ******************** \n");
  }
