...
/**
 ...
 Now (Feb 2013) would probably be a good time to regroup a bit, think about it
 and make it more general, more robust and just less "had-hoc"...
...
public:
  AliAnalysisTriggerScalers(const std::vector<int> &runs,
                            const char *source = "");
  ...
  void DrawFill(Int_t run1, Int_t run2, double ymin, double ymax,
                const char *label, Int_t color = 5);

  void DrawPeriods(Double_t ymin, Double_t ymax, Int_t color = 5);

  void GetCTPObjects(Int_t runNumber, AliTriggerConfiguration *&tc,
                     AliTriggerRunScalers *&trs, AliLHCData *&lhc) const;
  ...
  void
  GetLHCPeriodBoundaries(std::map<std::string, std::pair<int, int>> &periods);

  void GetLuminosityTriggerAndCrossSection(
  ...
  Float_t GetPauseAndConfigCorrection(Int_t runNumber,
                                      const char *triggerClassName);

  void GetPileUpFactor(Int_t runNumber, const char *triggerClassName,
                       Double_t purity, Double_t &value, Double_t &error);
  ...
  AliAnalysisTriggerScalerItem *GetTriggerScaler(Int_t runNumber,
                                                 const char *level,
                                                 const char *triggerClassName);

  TGraph *
  IntegratedLuminosityGraph(const char *triggerName,
                            const char *triggerClassNameForPACEstimate = "");

  TGraph *
  IntegratedLuminosityGraph(Int_t runNumber, const char *triggerClassName,
                            const char *triggerClassNameForPACEstimate = "");
  ...
  Int_t NumberOfInteractingBunches(const AliLHCData &lhc, Int_t runNumber,
                                   Bool_t mainSat = kFALSE,
                                   Bool_t silent = kFALSE) const;

  TGraph *PlotTrigger(const char *triggerClassName, const char *what,
                      bool draw = kTRUE);

  TGraph *PlotTriggerEvolution(const char *triggerClassName, const char *what,
                               bool draw = kTRUE, double *mean = 0x0,
                               bool removeZero = kFALSE,
                               std::vector<int> *runlist = 0x0);

  TGraph *PlotTriggerRatio(const char *triggerClassName1, const char *what1,
                           const char *triggerClassName2, const char *what2,
                           bool draw = kTRUE, Double_t factor = 1.0);

  TGraph *PlotTriggerRatioEvolution(const char *triggerClassName1,
                                    const char *what1,
                                    const char *triggerClassName2,
                                    const char *what2, bool draw = kTRUE,
                                    Double_t factor = 1.0);
  ...
  void PurgeRunList(std::vector<int> &purgedList, const char *triggerClassName,
                    Bool_t verbose = kTRUE);

private:
  Bool_t CheckRecord(const AliTriggerScalersRecordESD &record, UInt_t index,
                     ULong64_t refa, ULong64_t refb, UInt_t timelapse) const;
...
private:
  ...
  Bool_t
      fShouldCorrectForPileUp; // whether or not to correct scalers by pile-up
  TString fCrossSectionUnit; // cross-section unit (UB = microbarns) by default
  Bool_t fPurgeRunList; // make each PlotXXX call use its own runlist, purged
                        // from run w/o the requested trigger class
...
public:
  AliAnalysisTriggerScalerItem(Int_t runNumber, const char *level,
                               const char *dipoleCurrent,
                               const char *triggerClassName, ULong64_t value,
                               AliTriggerBCMask *mask = 0x0,
                               Int_t downscalingFactor = 1, time_t duration = 0)
      : fRunNumber(runNumber), fLevel(level), fDipoleCurrent(dipoleCurrent),
        fTriggerClassName(triggerClassName), fValue(value), fNofRuns(1),
        fTriggerBCMask(mask), fDS(downscalingFactor), fDuration(duration) {}

  AliAnalysisTriggerScalerItem(const char *triggerClassName, const char *level,
                               ULong64_t value, AliTriggerBCMask *mask = 0x0,
                               Int_t downscalingFactor = 1, time_t duration = 0)
      : fRunNumber(-1), fLevel(level), fDipoleCurrent("N/A"),
        fTriggerClassName(triggerClassName), fValue(value), fNofRuns(0),
        fTriggerBCMask(mask), fDS(downscalingFactor), fDuration(duration) {}
  ...
  Double_t ValueCorrectedForDownscale() const {
    return fValue * DownscalingFactor();
  }

  Int_t DownscalingFactor() const { return fDS; }

  Double_t Rate() const {
    return fDuration > 0 ? ValueCorrectedForDownscale() / fDuration : 0.0;
  }

  void Increment(ULong64_t val) {
    fValue += val;
    ++fNofRuns;
  }
...
private:
  ...
  operator=(const AliAnalysisTriggerScalerItem &rhs);
...
private:
  ...
  ClassDef(
      AliAnalysisTriggerScalerItem,
      5) // class to hold information about one scaler for one trigger class
};
