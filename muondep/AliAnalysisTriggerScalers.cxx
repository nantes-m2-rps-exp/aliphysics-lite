...
ClassImp(AliAnalysisTriggerScalers)
  ...
  Bool_t IsMainSatelliteCollision(const char *triggerClassName) {
    ...
    return (tcn.Contains("-S-") || tcn.Contains("-SC-") ||
            tcn.Contains("-SA-"));
  }
}

//_____________________________________________________________________________
AliAnalysisTriggerScalers::AliAnalysisTriggerScalers(Int_t runNumber,
                                                     const char *ocdbPath)
    : fRunList(), fOCDBPath(ocdbPath), fShouldCorrectForPileUp(kTRUE),
      fCrossSectionUnit("nb"), fPurgeRunList(kTRUE) {
...
AliAnalysisTriggerScalers::AliAnalysisTriggerScalers(
    const std::vector<int> &runs, const char *ocdbPath)
    : fRunList(), fOCDBPath(ocdbPath), fShouldCorrectForPileUp(kTRUE),
      fCrossSectionUnit("nb"), fPurgeRunList(kTRUE) {
...
//_____________________________________________________________________________
AliAnalysisTriggerScalers::AliAnalysisTriggerScalers(const std::set<int> &runs,
                                                     const char *ocdbPath)
    : fRunList(), fOCDBPath(ocdbPath), fShouldCorrectForPileUp(kTRUE),
      fCrossSectionUnit("nb"), fPurgeRunList(kTRUE) {
...
//_____________________________________________________________________________
AliAnalysisTriggerScalers::AliAnalysisTriggerScalers(const char *runlist,
                                                     const char *ocdbPath)
    : fRunList(), fOCDBPath(ocdbPath), fShouldCorrectForPileUp(kTRUE),
      fCrossSectionUnit("nb"), fPurgeRunList(kTRUE) {
...
//______________________________________________________________________________
Bool_t
AliAnalysisTriggerScalers::CheckRecord(const AliTriggerScalersRecordESD &record,
                                       UInt_t index, ULong64_t refb,
                                       ULong64_t refa, UInt_t timelapse) const {
...
//______________________________________________________________________________
void AliAnalysisTriggerScalers::DrawFill(Int_t run1, Int_t run2, double ymin,
                                         double ymax, const char *label,
                                         Int_t color) {
  // Draw a yellow box to indicate a run range

  AliDebugClass(1, Form("RUN1 %09d RUN2 %09d YMIN %e YMAX %e %s", run1, run2,
                        ymin, ymax, label));
  TBox *b = new TBox(run1 * 1.0, ymin, run2 * 1.0, ymax);
  b->SetFillColor(color);
  b->Draw();
  TText *text =
      new TText((run1 + run2) / 2.0, ymin + (ymax - ymin) * 0.85, label);
...
void AliAnalysisTriggerScalers::DrawFills(Double_t ymin, Double_t ymax,
  ...
  /// Note that this method will scan the OCDB to get the run -> fill number
  /// relationship, so it's better in this case to use a local copy of the OCDB
  /// if you have one. Otherwise it will be long.
  ///
...
//_____________________________________________________________________________
void AliAnalysisTriggerScalers::DrawPeriods(Double_t ymin, Double_t ymax,
                                            Int_t color) {
...
void AliAnalysisTriggerScalers::GetCTPObjects(Int_t runNumber,
  ...
  /// we can get those objects from either OCDB or run based container in
  /// AODs/ESDs

  tc = static_cast<AliTriggerConfiguration *>(
      GetOCDBObject("GRP/CTP/Config", runNumber));
  trs = static_cast<AliTriggerRunScalers *>(
      GetOCDBObject("GRP/CTP/Scalers", runNumber));
...
//_____________________________________________________________________________
void AliAnalysisTriggerScalers::GetFillBoundaries(
    std::map<int, std::pair<int, int>> &fills) {
...
Int_t AliAnalysisTriggerScalers::GetFillNumberFromRunNumber(Int_t runNumber) {
  ...
  AliLHCData *lhcData =
      static_cast<AliLHCData *>(GetOCDBObject("GRP/GRP/LHCData", runNumber));
  if (lhcData) {
...
//______________________________________________________________________________
TObject *AliAnalysisTriggerScalers::GetOCDBObject(const char *path,
...
//______________________________________________________________________________
TString
AliAnalysisTriggerScalers::GetLHCPeriodFromRunNumber(Int_t runNumber) const {
  /// Get a list of LHC periods from a list of run numbers

  AliGRPObject *grp =
      static_cast<AliGRPObject *>(GetOCDBObject("GRP/GRP/Data", runNumber));
...
//_____________________________________________________________________________
void AliAnalysisTriggerScalers::GetLuminosityTriggerAndCrossSection(
    Int_t runNumber, TString &lumiTriggerClassName,
    Double_t &lumiTriggerCrossSection,
    Double_t &lumiTriggerCrossSectionError) const {
  /// For one given run, get the trigger class name to be used as the
  /// luminometer, and its corresponding cross-section (in mb)
  ///
  ...
  if (period.BeginsWith("LHC11")) {
    ...
    if (period.BeginsWith("LHC15i") || period.BeginsWith("LHC15j") ||
        period.BeginsWith("LHC15k") || period.BeginsWith("LHC15l")) {
      lumiTriggerClassName = "C0TVX-B-NOPF-CENTNOTRD";
    }
    ...
    if (runNumber >= 280141 && runNumber <= 280235) {
      // from
      // http://aliqaevs.web.cern.ch/aliqaevs/data/2017/runLevelEventStatQA.C
  ...
  } else if (period.BeginsWith("LHC18m")) {
    lumiTriggerClassName = "C0TVX-B-NOPF-CENTNOTRD";
    lumiTriggerCrossSection = 30.0;
  } else {
...
//_____________________________________________________________________________
void AliAnalysisTriggerScalers::GetLHCPeriodBoundaries(
    std::map<std::string, std::pair<int, int>> &periods) {
  /// Given a list of runs get the fills they are in
...
//______________________________________________________________________________
Int_t AliAnalysisTriggerScalers::GetTriggerInput(Int_t runNumber,
                                                 const char *inputname) {
  /// Get one trigger input for a given run
  AliTriggerConfiguration *tc = static_cast<AliTriggerConfiguration *>(
      GetOCDBObject("GRP/CTP/Config", runNumber));
  if (!tc)
    return -1;
  ...
  AliTriggerInput *ti =
      static_cast<AliTriggerInput *>(inputs.FindObject(inputname));
...
//______________________________________________________________________________
Float_t AliAnalysisTriggerScalers::GetPauseAndConfigCorrection(
    Int_t runNumber, const char *triggerClassName) {
  /// Tries to estimate the duration of the Pause and Configure phase(s) in a
  /// given run Probably not really accurate for the moment though.
  ...
  AliTriggerRunScalers *trs = static_cast<AliTriggerRunScalers *>(
      GetOCDBObject("GRP/CTP/Scalers", runNumber));
  CorrectRunScalers(trs);
  ...
  while ((record = static_cast<AliTriggerScalersRecordESD *>(next()))) {
    const AliTriggerScalersESD *scaler =
        record->GetTriggerScalersForClass(classIndex);
...
//______________________________________________________________________________
void AliAnalysisTriggerScalers::ShowPileUpFactors(const char *triggerClassName,
                                                  Double_t purity) {
  /// Printout the pile-up factors for the runlist, including some purity if
  /// needed

  if (purity <= 0.0) {
    AliError(Form("Cannot work with purity=%f for trigger %s. Should be "
                  "strictly positive",
                  purity, triggerClassName));
    return;
  }
  ...
  std::vector<std::string>
      lines; // put the data into lines so we can get the OCDB access printout
             // grouped before the important stuff
  ...
  for (std::vector<int>::const_iterator it = rl.begin(); it != rl.end(); ++it) {
    ...
    str << Form(
        "RUN %6d PERIOD %6s PILE-UP CORRECTION FACTOR (mu/(1-exp(-mu)) = %7.4f",
        *it, period.Data(), value);
...
//______________________________________________________________________________
void AliAnalysisTriggerScalers::GetPileUpFactor(Int_t runNumber,
  ...
  if (purity <= 0.0) {
    AliError(Form("Cannot work with purity=%f for trigger %s in run %d. Should "
                  "be strictly positive",
                  purity, triggerClassName, runNumber));
    return;
  }
  ...
  const AliTriggerScalersRecordESD *begin =
      static_cast<const AliTriggerScalersRecordESD *>(scalers->First());
  const AliTriggerScalersRecordESD *end =
      static_cast<const AliTriggerScalersRecordESD *>(scalers->Last());

  time_t duration = TMath::Nint((end->GetTimeStamp()->GetBunchCross() -
                                 begin->GetTimeStamp()->GetBunchCross()) *
                                AliTimeStamp::fNanosecPerBC * 1E-9);
  ...
  AliAnalysisTriggerScalerItem *item =
      GetTriggerScaler(runNumber, "L0B", triggerClassName);
  if (!item) {
    AliError(Form("Could not get L0B for trigger %s in run %d",
                  triggerClassName, runNumber));
    return;
  }
  ...
  if (nbcx <= 0.0) {
    AliError(Form("Cannot work with nbcx=%d for trigger %s in run %d. Should "
                  "be strictly positive",
                  nbcx, triggerClassName, runNumber));
    return;
  }
  ...
  if (itemValue <= 0.0) {
    AliError(Form("Cannot work with value=%f for trigger %s in run %d. Should "
                  "be strictly positive",
                  itemValue, triggerClassName, runNumber));
    return;
  }
...
//______________________________________________________________________________
void AliAnalysisTriggerScalers::CorrectRunScalers(
    AliTriggerRunScalers *trs) const {
...
TString
AliAnalysisTriggerScalers::DefaultTitle(std::vector<int> *runlist) const {
...
//__________________________________________________________________________________________________
void AliAnalysisTriggerScalers::GetTriggerClassesForRun(
    Int_t runNumber, TString &classes) const {
...
AliAnalysisTriggerScalers::GetTriggerScaler(Int_t runNumber, const char *level,
                                            const char *triggerClassName) {
  ...
  TString diCurrent(Form("L3:%5.0f;DIP:%5.0f [L3:%d;DIP:%d]",
                         ...
                         grp->GetL3Polarity(), grp->GetDipolePolarity()));
  ...
  AliTriggerClass *triggerClass =
      static_cast<AliTriggerClass *>(trClasses.FindObject(triggerClassName));
  if (!triggerClass) {
    return 0x0;
  }

  Int_t index =
      GetIndex(triggerClass->GetMask(), triggerClass->GetMaskNext50());
  ...
  if (swhat.BeginsWith("L0A")) {
    ...
    if (l0a) {
      value = (send->GetLOCB() - sbegin->GetLOCB()) *
              (send->GetL1CB() - sbegin->GetL1CB()) / l0a;
    }
  ...
  // FIXME: get the downscaling factor here for all cases (only BC1 supported
  // here so far)
  ...
  return new AliAnalysisTriggerScalerItem(
      runNumber, swhat.Data(), diCurrent.Data(), triggerClass->GetName(), value,
      triggerClass->GetBCMask(), ds, duration);
}

//______________________________________________________________________________
TGraph *AliAnalysisTriggerScalers::IntegratedLuminosityGraph(
    const char *triggerClassName, const char *triggerClassNameForPACEstimate) {
  ...
  for (std::vector<int>::size_type i = 0; i < fRunList.size(); ++i) {
    TGraph *g = IntegratedLuminosityGraph(fRunList[i], triggerClassName,
                                          triggerClassNameForPACEstimate);
...
TGraph *AliAnalysisTriggerScalers::IntegratedLuminosityGraph(
    Int_t runNumber, const char *triggerClassName,
    const char *triggerClassNameForPACEstimate) {
  ...
  // and should be the trigger class with the higher and non-zero L0A rate
  // (except during PACs)
  //
  ...
  GetLuminosityTriggerAndCrossSection(runNumber, lumiTriggerClassName,
                                      lumiTriggerCrossSection,
                                      lumiTriggerCrossSectionError);

  AliTriggerClass *lumiTriggerClass = static_cast<AliTriggerClass *>(
      triggerClasses.FindObject(lumiTriggerClassName));

  if (!lumiTriggerClass) {
    AliError(Form("Could not find lumiTriggerClass=%s",
                  lumiTriggerClassName.Data()));
    return 0x0;
  }

  if (lumiTriggerCrossSection == 0.0) {
    AliError(Form("Could not get cross-section for lumiTriggerClass=%s",
                  lumiTriggerClassName.Data()));
    return 0x0;
  }

  AliTriggerClass *triggerClass = static_cast<AliTriggerClass *>(
      triggerClasses.FindObject(triggerClassName));
  ...
  if (ShouldCorrectForPileUp()) {
    ...
    if (nbcx <= 0) {
      AliError("Could not get the number of bunches, so won't be able to "
               "correct for pile-up");
    }
  }
  ...
  classIndices[0] =
      GetIndex(lumiTriggerClass->GetMask(), lumiTriggerClass->GetMaskNext50());
  classIndices[1] =
      GetIndex(triggerClass->GetMask(), triggerClass->GetMaskNext50());
  ...
  if (sameClass && strlen(triggerClassNameForPACEstimate) > 0) {
    AliTriggerClass *triggerClassForPACEstimate =
        static_cast<AliTriggerClass *>(
            triggerClasses.FindObject(triggerClassNameForPACEstimate));
    if (!triggerClassForPACEstimate) {
      AliError(Form("Could not find triggerClassForPACEstimate=%s. Will not "
                    "correct for PAC durations",
                    triggerClassNameForPACEstimate));
      return 0x0;
    }
    classIndices[1] = GetIndex(triggerClassForPACEstimate->GetMask(),
                               triggerClassForPACEstimate->GetMaskNext50());
  ...
  while ((record = static_cast<AliTriggerScalersRecordESD *>(nextScaler()))) {
    ...
    Bool_t ok = sameClass || CheckRecord(*record, classIndices[1], refl0b[1],
                                         refl0a[1], timelapse);
    ...
    for (Int_t i = 0; i < 2; ++i) {
      const AliTriggerScalersESD *scaler =
          record->GetTriggerScalersForClass(classIndices[i]);
...
void AliAnalysisTriggerScalers::IntegratedLuminosity(const char *triggerList,
  ...
  if (out) {
    ...
    (*out) << Form("LHC delivered (%s^-1) per fill ", fCrossSectionUnit.Data())
           << sep
           << Form("LHC delivered (%s^-1 integrated)", fCrossSectionUnit.Data())
           << sep;
  ...
  for (std::vector<int>::size_type i = 0; i < fRunList.size(); ++i) {
    ...
    AliAnalysisTriggerScalerItem *lumiB =
        GetTriggerScaler(runNumber, "L0B", lumiTriggerClassName.Data());

    if (!lumiB) {
      AliError(Form("Did not find lumiTrigger %s for run %09d",
                    lumiTriggerClassName.Data(), runNumber));
      continue;
    }
    ...
    while ((trigger = static_cast<TObjString *>(nextTrigger()))) {
      ...
      if (muTriggerClassName.Contains("CMUL8"))
        ++n;
      if (muTriggerClassName.Contains("CMUL7"))
        ++n;

      if (n > 1) {
        AliError(Form("More than 1 relevant trigger class found for run %09d ! "
                      "Check that !",
                      runNumber));
        trClasses.Print();
        continue;
      ...
      AliAnalysisTriggerScalerItem *muonA =
          GetTriggerScaler(runNumber, "L0A", muTriggerClassName.Data());
      AliAnalysisTriggerScalerItem *muonB =
          GetTriggerScaler(runNumber, "L0B", muTriggerClassName.Data());
      ...
      if (muonB->ValueCorrectedForDownscale() > 0) {
        ratio = muonA->ValueCorrectedForDownscale() /
                muonB->ValueCorrectedForDownscale();
      }
      ...
      if (muTriggerClassName.Contains("CMSH")) {
        pacCorrection =
            GetPauseAndConfigCorrection(runNumber, muTriggerClassName.Data());
      }

      lumiPerTrigger[muTriggerClassName.Data()] += ratio;
      durationPerTrigger[muTriggerClassName.Data()] += duration;
      lumiPerFillPerTrigger[currentFillNumber][muTriggerClassName.Data()] +=
          ratio;
    }

    lumiPerTrigger[lumiTriggerClassName.Data()] +=
        lumiB->ValueCorrectedForDownscale() / lumiSigma;
    durationPerTrigger[lumiTriggerClassName.Data()] += duration;
    lumiPerFillPerTrigger[currentFillNumber][lumiTriggerClassName.Data()] +=
        lumiB->ValueCorrectedForDownscale() / lumiSigma;

    /*
     TString lumiPACCorrected(Form("%s(-PAC)",lumiTriggerClassName.Data()));

     lumiPerTrigger[lumiPACCorrected.Data()] +=
     pacCorrection*lumiB->ValueCorrectedForDownscale()/lumiSigma;
     durationPerTrigger[lumiPACCorrected.Data()] += duration;
     lumiPerFillPerTrigger[currentFillNumber][lumiPACCorrected.Data()] +=
     pacCorrection*lumiB->ValueCorrectedForDownscale()/lumiSigma;
     */
  ...
  AliInfo(
      Form("Integrated lumi %7.4f %s^-1", intLumi, fCrossSectionUnit.Data()));

  std::map<std::string, float>::const_iterator it;

  for (it = lumiPerTrigger.begin(); it != lumiPerTrigger.end(); ++it) {
    AliInfo(Form("Trigger %30s Lumi %10.4f %s^-1 duration %10ld s",
                 it->first.c_str(), it->second, fCrossSectionUnit.Data(),
                 durationPerTrigger[it->first]));
  }
  ...
  if (out) {
    ...
    for (fit = lumiPerFillPerTrigger.begin();
         fit != lumiPerFillPerTrigger.end(); ++fit) {
      int fill = fit->first;
      ...
      (*out) << "0" << sep << "0" << sep << "0" << sep << "0" << sep
             << std::endl; // LHC per fill, LHC integrated, lumi tot muon ,
                           // efficiency
    }
  }
  //
...
TGraph *AliAnalysisTriggerScalers::MakeGraph(const std::vector<int> &vx,
  ...
  if (!(vx.size() == vex.size() && vx.size() == vy.size() &&
        vx.size() == vey.size())) {
...
Double_t AliAnalysisTriggerScalers::Millibarn2CrossSectionUnit() const {
  ...
  if (fCrossSectionUnit == "pb") {
    ...
    AliError(Form("Don't know how to go from mb to %s : will use 1.0",
                  fCrossSectionUnit.Data()));
  }
  return csMult;
}
...
Double_t AliAnalysisTriggerScalers::Mu(Double_t L0B, Double_t Nb) {
  ...
  Double_t p0 =
      1 -
      L0B / (Nb * 11245.0); // proba to get *no* collision per bunch crossing

  return -TMath::Log(p0);
}

//______________________________________________________________________________
Int_t AliAnalysisTriggerScalers::NumberOfInteractingBunches(
    const AliLHCData &lhc, Int_t runNumber, Bool_t mainSat,
    Bool_t silent) const {
  ...
  if (mainSat) {
    ...
    for (Int_t i = 0; (val != 0x0) && i < val->GetSizeTotal(); ++i) {
      if (val->GetValue(i) < 0)
        ++numberOfInteractingBunches;
    }

    for (Int_t i = 0; (valm != 0x0) && i < valm->GetSizeTotal(); ++i) {
      if (valm->GetValue(i) < 0)
  ...
  if (!numberOfInteractingBunches) {
    if (!numberOfInteractingBunchesMeasured) {
      if (runNumber == 245996 || runNumber == 246001 || runNumber == 246003) {
        AliWarning(
            Form("Could not get the number of interacting bunches for run %d "
                 "from LHCData. Using a hard coded constant of 362",
                 runNumber));
        return 362;
      }
      AliFatal(
    ...
    AliDebug(1, Form("Declared bunch filling is empty, but measured one has %d "
                     "interacting bunches, overriding to measurement",
                     numberOfInteractingBunchesMeasured));
  ...
  if (numberOfInteractingBunches != numberOfInteractingBunchesMeasured ||
    ...
    Int_t delta = TMath::Max(numberOfInteractingBunches -
                                 numberOfInteractingBunchesMeasured,
                             numberOfInteractingBunches - nIBM2);

    if (1.0 * TMath::Abs(delta) / numberOfInteractingBunches >
        0.05) // more than 5% difference
    {
      if (!silent) {
        AliWarning(Form(
            "Got some different number of interacting bunches for fill %d "
            "run %d ! NumberOfInteractingBunches=%3d "
            "NumberOfInteractingBunchesMeasured=%3d NIBM2=%3d. Will use %d",
            lhc.GetFillNumber(), runNumber, numberOfInteractingBunches,
            numberOfInteractingBunchesMeasured, nIBM2,
            numberOfInteractingBunches));
      }
    } else {
      AliDebug(
          1,
          Form("Got some different number of interacting bunches for fill %d "
               "run %d ! NumberOfInteractingBunches=%3d "
               "NumberOfInteractingBunchesMeasured=%3d NIBM2=%3d. Will use %d",
               lhc.GetFillNumber(), runNumber, numberOfInteractingBunches,
               numberOfInteractingBunchesMeasured, nIBM2,
...
TGraph *AliAnalysisTriggerScalers::PlotTrigger(const char *triggerClassName,
  ...
  for (std::vector<int>::size_type i = 0; i < runlist.size(); ++i) {
    ...
    if (swhat == "L0AOVERB") {
      AliAnalysisTriggerScalerItem *a =
          GetTriggerScaler(runNumber, "L0A", triggerClassName);

      if (!a) {
        AliWarning(Form("Could not get L0A for trigger %s for run %d",
                        triggerClassName, runNumber));
        continue;
      }

      AliAnalysisTriggerScalerItem *b =
          GetTriggerScaler(runNumber, "L0B", triggerClassName);

      if (!b) {
        AliWarning(Form("Could not get L0B for trigger %s for run %d",
                        triggerClassName, runNumber));
        continue;
      }
      ...
      AliLHCData *lhc = static_cast<AliLHCData *>(
          GetOCDBObject("GRP/GRP/LHCData", runNumber));

      Int_t numberOfInteractingBunches =
          NumberOfInteractingBunches(*lhc, runNumber, mainSat);

      value = Mu(b->ValueCorrectedForDownscale() / b->Duration(),
                 numberOfInteractingBunches);
      ...
      double mu = Mu(b->ValueCorrectedForDownscale() / b->Duration(),
                     numberOfInteractingBunches);
      ...
      value =
          b->ValueCorrectedForDownscale() / b->Duration() / (11245.0 * nbcx);
    } else if (swhat == "L0BEVAL") {
      AliAnalysisTriggerScalerItem *l0a =
          GetTriggerScaler(runNumber, "L0A", triggerClassName);
      ...
      AliAnalysisTriggerScalerItem *l1b =
          GetTriggerScaler(runNumber, "L1B", triggerClassName);

      if (!l1b) {
        AliWarning(Form("Could not get L1B for trigger %s for run %d",
                        triggerClassName, runNumber));
        continue;
      }

      AliAnalysisTriggerScalerItem *l0b =
          GetTriggerScaler(runNumber, "L0B", triggerClassName);
      ...
      if (l0a->ValueCorrectedForDownscale() > 0) {
        value = l0b->ValueCorrectedForDownscale() *
                l1b->ValueCorrectedForDownscale() /
                l0a->ValueCorrectedForDownscale();
      }
    } else {
      AliAnalysisTriggerScalerItem *s =
          GetTriggerScaler(runNumber, what, triggerClassName);
...
//______________________________________________________________________________
TGraph *AliAnalysisTriggerScalers::PlotTriggerEvolution(
    const char *triggerClassName, const char *what, bool draw, double *mean,
    bool removeZeros, std::vector<int> *commonrunlist) {
  ...
  /// - L0AOVERB (L0A/L0B)
  /// - L1BOVERL0A (L1B/L0A)
  /// - mu ( = -TMath::Log( 1 - P(0) ) where P(0) is the proba to have zero
  /// collisions in a bunch crossing)
  /// - pileupfactor = mu/(1-exp(-mu)) : the factor to apply to correct the
  /// cint1b count rate
  /// - vsnb = L0B/(NumberOfInteractingBunches*11245)
  /// - NBCX = NumberOfInteractingBunches
  /// - L0BEVAL = evaluation of an equivalent L0B rate for a L1 trigger = L0B x
  /// L1B / L0A
  ...
  if (commonrunlist) {
    ...
    PurgeRunList(*runlist, triggerClassName);
    AliInfo(Form("Will use %lu runs (with the requested classes) among the %lu "
                 "requested",
                 runlist->size(), fRunList.size()));
  }
  ...
  for (std::vector<int>::size_type iRun = 0; iRun < runlist->size(); ++iRun) {
    ...
    if (!tc || !trs || !lhc) {
      AliError(
          Form("Could not get some data from OCDB for run %09d : "
               "TriggerConfiguration : %p TriggerRunScalers : %p LHCData : %p",
               runNumber, tc, trs, lhc));
      continue;
    }
    ...
    while ((triggerClass = static_cast<AliTriggerClass *>(next()))) {
      ...
      if (!TString(triggerClass->GetName()).Contains(triggerClassName))
        continue;

      Bool_t mainSat = IsMainSatelliteCollision(triggerClass->GetName());

      Int_t numberOfInteractingBunches =
          0; // NumberOfInteractingBunches(*lhc,runNumber,mainSat);

      if (swhat.Contains("MU") || swhat.Contains("PILEUPFACTOR") ||
          swhat.Contains("VSNB") || swhat.Contains("NBCX")) {
        numberOfInteractingBunches =
            NumberOfInteractingBunches(*lhc, runNumber, mainSat);
      }
      ...
      while (
          (record = static_cast<AliTriggerScalersRecordESD *>(nextScaler()))) {
        const AliTriggerScalersESD *scaler =
            record->GetTriggerScalersForClass(index);
        ...
        if (removeZeros &&
            (l0b <= 2 || (l0a <= 2 && l0a != 0) || timelapse <= 9)) {
          AliInfo(
              Form("Skipping point for %s l0b %llu l0a %llu timelapse %d ts %s",
          ...
          if (value > 1E-3) {
            ...
            if (delta > 0.3) // more than 30% difference
            {
              if (low.size() > 2) {
                ...
                for (std::vector<double>::size_type j = 0; j < low.size();
                     ++j) {
                  std::cout << low[j] << ",";
                }
                std::cout << std::endl;
...
//______________________________________________________________________________
TGraph *AliAnalysisTriggerScalers::PlotTriggerRatio(
    const char *triggerClassName1, const char *what1,
    const char *triggerClassName2, const char *what2, Bool_t draw,
    Double_t factor) {
  ...
  AliInfo(Form(
      "Will use %lu runs (with the requested classes) among the %lu requested",
      runlist.size(), fRunList.size()));

  for (std::vector<int>::size_type i = 0; i < runlist.size(); ++i) {
    Int_t runNumber = runlist[i];

    AliAnalysisTriggerScalerItem *s1 =
        GetTriggerScaler(runNumber, what1, triggerClassName1);
    AliAnalysisTriggerScalerItem *s2 =
        GetTriggerScaler(runNumber, what2, triggerClassName2);
    ...
    if (s2->ValueCorrectedForDownscale() > 0) {
      ratio =
          s1->ValueCorrectedForDownscale() / s2->ValueCorrectedForDownscale();
      if (s1->ValueCorrectedForDownscale() > 0) {
        ratioErr = 1.0 / (s1->ValueCorrectedForDownscale() *
                          s1->ValueCorrectedForDownscale());
        ratioErr += 1.0 / (s2->ValueCorrectedForDownscale() *
                           s2->ValueCorrectedForDownscale());
    ...
    AliDebug(
        1,
        Form(
            "RUN %09d %20s (%s) %12llu (%5d) %20s (%s) %12llu (%5d) R %e RE %e",
            runNumber, triggerClassName1, what1, s1->Value(),
            s1->DownscalingFactor(), triggerClassName2, what2, s2->Value(),
            s2->DownscalingFactor(), ratio, ratioErr));
  }
  ...
  title += Form("%s-%s-OVER-%s-%s", triggerClassName1, what1, triggerClassName2,
                what2);
...
//______________________________________________________________________________
TGraph *AliAnalysisTriggerScalers::PlotTriggerRatioEvolution(
    const char *triggerClassName1, const char *what1,
    const char *triggerClassName2, const char *what2, bool draw,
    Double_t factor) {
  ...
  if (fPurgeRunList) {
    ...
    runlist = new std::vector<int>(
        AliAnalysisRunList::GetRunsInCommon(runlist1, runlist2));
  ...
  TGraph *g1 = PlotTriggerEvolution(triggerClassName1, what1, kFALSE, 0x0,
                                    kFALSE, runlist);
  TGraph *g2 = PlotTriggerEvolution(triggerClassName2, what2, kFALSE, 0x0,
                                    kFALSE, runlist);
...
//______________________________________________________________________________
void AliAnalysisTriggerScalers::PurgeRunList(std::vector<int> &purgedList,
                                             const char *triggerClassName,
                                             Bool_t verbose) {
  /// Copy into purgedList all the runs from fRunList which have the
  /// triggerClassName defined
  ...
  for (std::vector<int>::size_type i = 0; i < fRunList.size(); ++i) {
    ...
    while ((str = static_cast<TObjString *>(next())) && !ok) {
      if (str->String().CompareTo(triggerClassName, TString::kIgnoreCase) ==
          0) {
        ok = kTRUE;
      }
    }
    ...
    delete a;
    if (ok) {
      if (std::find(purgedList.begin(), purgedList.end(), runNumber) ==
          purgedList.end()) {
        purgedList.push_back(runNumber);
      }
    } else {
      if (verbose) {
        AliWarning(Form("Run %09d does not have trigger class %s", runNumber,
                        triggerClassName));
      }
    }
  }
...
void AliAnalysisTriggerScalers::SetOCDBPath(const char *path) {
  ...
  if (strlen(path) == 0) {
    ...
    spath.Form("/cvmfs/alice-ocdb.cern.ch/calibration/data/%d/OCDB",
               now.GetYear());
...
ClassImp(AliAnalysisTriggerScalerItem)
    ...
    //______________________________________________________________________________
    const char *AliAnalysisTriggerScalerItem::BCMaskName() const {
  // return bc mask name
  if (BCMask())
    return BCMask()->GetName();
  else
    return "";
}
...
Int_t AliAnalysisTriggerScalerItem::Compare(const TObject *obj) const {
  ...
  const AliAnalysisTriggerScalerItem *s =
      static_cast<const AliAnalysisTriggerScalerItem *>(obj);
...
void AliAnalysisTriggerScalerItem::Print(Option_t *opt) const {
  ...
  if (RunNumber() > 0) {
    ...
    if (sopt.Contains("FULL")) {
      ...
      std::cout << Form("RUN %6d CLASS %24s (%5s %4d BCs) SCALER %s %12llu DS "
                        "%6d DURATION %ld",
                        RunNumber(), TriggerClassName(), BCMaskName(),
                        BCMask() ? BCMask()->GetNUnmaskedBCs() : 0, Level(),
                        Value(), DownscalingFactor(), Duration())
                << std::endl;
    }
  } else {
    std::cout << Form("CLASS %24s SCALER %20llu NRUNS %d", TriggerClassName(),
                      Value(), NofRuns())
              << std::endl;
  }
}
