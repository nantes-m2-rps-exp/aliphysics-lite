...
#include <map>
    ...
    AliAnalysisMuMuResult::AliAnalysisMuMuResult(
        ...
        AliAnalysisMuMuResult::EResultMergingMethod mergingMethod)
    : TNamed(name, title), fSubResults(0x0), fMap(0x0), fMother(0x0),
      fKeys(0x0), fWeight(0.0), fAlias(), fSubResultsToBeIncluded(0x0),
      fResultMergingMethod(mergingMethod), fIsValid(kTRUE), fVisibleKeys(0x0) {
  /// default ctor
}

//_____________________________________________________________________________
AliAnalysisMuMuResult::AliAnalysisMuMuResult(const AliAnalysisMuMuResult &rhs)
    : TNamed(rhs), fSubResults(0x0), fMap(0x0), fMother(0x0), fKeys(0x0),
      fWeight(rhs.fWeight), fAlias(), fSubResultsToBeIncluded(0x0),
      fIsValid(kTRUE), fVisibleKeys(0x0) {
  /// copy ctor
  ...
  if (rhs.fSubResultsToBeIncluded) {
    fSubResultsToBeIncluded =
        static_cast<TList *>(rhs.fSubResultsToBeIncluded->Clone());
  }
...
//_____________________________________________________________________________
Double_t AliAnalysisMuMuResult::ErrorAB(Double_t a, Double_t aerr, Double_t b,
                                        Double_t berr) {
...
//_____________________________________________________________________________
Double_t AliAnalysisMuMuResult::ErrorABC(Double_t a, Double_t aerr, Double_t b,
                                         Double_t berr, Double_t c,
                                         Double_t cerror) {
...
//_____________________________________________________________________________
Double_t AliAnalysisMuMuResult::ErrorABCD(Double_t a, Double_t aerr, Double_t b,
                                          Double_t berr, Double_t c,
                                          Double_t cerror, Double_t d,
                                          Double_t derror) {
...
//_____________________________________________________________________________
Double_t AliAnalysisMuMuResult::ErrorABCDE(Double_t a, Double_t aerr,
                                           Double_t b, Double_t berr,
                                           Double_t c, Double_t cerror,
                                           Double_t d, Double_t derror,
                                           Double_t ee, Double_t eeerror) {
  /// Compute the quadratic sum of 4 errors
...
//_____________________________________________________________________________
Double_t AliAnalysisMuMuResult::GetErrorStat(const char *name,
  ...
  if (strlen(subResultName) > 0) {
    if (!fSubResults) {
      AliError(Form("No subresult from which I could get the %s one...",
                    subResultName));
      return TMath::Limits<Double_t>::Max();
    }
    AliAnalysisMuMuResult *sub = static_cast<AliAnalysisMuMuResult *>(
        fSubResults->FindObject(subResultName));
    if (!sub) {
  ...
  if (fMap) {
    ...
    if (p) {
      TParameter<double> *val =
          static_cast<TParameter<double> *>(p->At(kErrorStat));
      return val->GetVal();
    }
  }
  ...
  if (fResultMergingMethod == kMean) {
    ...
    while ((r = static_cast<AliAnalysisMuMuResult *>(next()))) {
      ...
      if (IsIncluded(r->Alias()) && r->HasValue(name)) {
        ...
        // The weight for each subresult is the same (=1.), since the data
        // sample is always the same and just the fit function changes among
        // subresults. Before subresults were weighted with 1./err/err/wstat;
        // wstat was not there before and was introduced to remove the
        // dependence of the error with the Nof extracted Jpsi (valid only for
        // counts results with different data samples and not for <pt>...)

        // NOT OUTDATED COMMENT : the stat. error is just a weighted mean of all
        // the stat. error.

        Int_t fitStatus =
            r->HasValue("FitResult") ? r->GetValue("FitResult") : 0;
        Int_t covStatus =
            r->HasValue("CovMatrixStatus") ? r->GetValue("CovMatrixStatus") : 3;
        Int_t chi2 =
            r->HasValue("FitChi2PerNDF") ? r->GetValue("FitChi2PerNDF") : 1;
        // Select only Fit that converge
        if ((fitStatus != 0 && fitStatus != 4000) ||
            chi2 > 2.5 /*|| covStatus!=3*/) {
          AliDebug(1,
                   Form("Fit %s excluded (FitResult = %d | Cov. Mat. = %d)\n",
        ...
        AliDebug(1, Form(" ----> Weight for subResults %s = %f \n",
                         r->GetName(), w));
        AliDebug(
            1, Form("FitStatus for subresult : %f", r->GetValue("FitStatus")));
        if (!(err > 0.0)) {
          continue;
...
//_____________________________________________________________________________
Double_t AliAnalysisMuMuResult::GetRMS(const char *name,
                                       const char *subResultName) const {
  ...
  if (fMap) {
    ...
    if (p) {
      ...
      return val ? val->GetVal() : 0.0; // val can be null for old results which
                                        // did not have the rms set
    }
  }
  ...
  while ((r = static_cast<AliAnalysisMuMuResult *>(next()))) {
    if (IsIncluded(r->Alias()) && r->HasValue(name)) {
      ...
      // Before subresults were weighted with 1./err/err/wstat; wstat was not
      // there before and was introduced to remove the dependence of the error
      // with the Nof extracted Jpsi (valid only for counts results with
      // different data samples and not for <pt>...)
  ...
  AliDebug(
      1,
      Form(" ----> v1 %e v1*v1 %e v2 %e -> biased %e unbiased %e (ratio %e) \n",
...
//_____________________________________________________________________________
Double_t AliAnalysisMuMuResult::GetValue(const char *name,
                                         const char *subResultName) const {
  ...
  if (fMap) {
    ...
    if (p) {
      TParameter<double> *val =
          static_cast<TParameter<double> *>(p->At(kValue));
      return val->GetVal();
    }
  }
  ...
  if (fResultMergingMethod == kMean) {
    ...
    while ((r = static_cast<AliAnalysisMuMuResult *>(next()))) {
      if (IsIncluded(r->Alias()) && r->HasValue(name)) {
        // The weight for each subresult is the same (=1.), since the data
        // sample is always the same and just the fit function changes among
        // subresults. Before subresults were weighted with e =
        // r->GetErrorStat(name)/TMath::Sqrt(r->GetValue(name));
        // The Sqrt(r->GetValue(name)) was not there before and was introduced
        // to remove the dependence of the error with the Nof extracted Jpsi
        // (valid only for counts results with different data samples and not for
        // <pt>...)
        ...
        AliDebug(1, Form(" ----> Weight for subResults %s = %f \n",
                         r->GetName(), e));
        // Double_t e2 = e*e;
        if (!(r->GetErrorStat(name) > 0.0) || (r->GetValue("FitStatus") != 0))
          continue;
              /*e2 = TMath::Sqrt(r->GetValue(name));*/ // If the error is not
                                                       // correct (fit not good)
                                                       // we skip the subresult
...
//_____________________________________________________________________________
Int_t AliAnalysisMuMuResult::HasValue(const char *name,
                                      const char *subResultName) const {
  /// Whether this result (or subresult if subResultName is provided) has a
  /// property named "name"
  ///
  /// When having subresults, return the number of subresults that have this
  /// value
  ///
...
Bool_t AliAnalysisMuMuResult::IsIncluded(const TString &alias) const {
  /// whether that subresult alias should be included when computing means,
  /// etc...
...
Long64_t AliAnalysisMuMuResult::Merge(TCollection *list) {
  ...
  while ((currObj = next())) {
    AliAnalysisMuMuResult *result =
        dynamic_cast<AliAnalysisMuMuResult *>(currObj);
    if (!result) {
      AliFatal(Form(
          "object named \"%s\" is a %s instead of an AliAnalysisMuMuResult!",
          currObj->GetName(), currObj->ClassName()));
      continue;
    }
  ...
  while ((key = static_cast<TObjString *>(nextKey()))) {
    ...
    while ((currObj = next())) {
      ...
      if (!result->HasValue(key->String())) {
        AliError(Form("Did not find key %s in of the result to merge",
                      key->String().Data()));
        continue;
      }
      ...
      e2 += result->GetErrorStat(key->String()) *
            result->GetErrorStat(key->String()) * w2;
    }

    Set(key->String(), value, TMath::Sqrt(e2));
  }
...
void AliAnalysisMuMuResult::Print(Option_t *opt) const {
  ...
  std::cout << Form("%s %s %s", GetName(), GetTitle(),
  ...
  while ((key = static_cast<TObjString *>(next()))) {
    if (fVisibleKeys && !fVisibleKeys->FindObject(key->String()))
      continue;

    if (nsub == 0 || nsub == HasValue(key->String().Data())) {
      PrintValue(key->String().Data(), pot.Data(), GetValue(key->String()),
                 GetErrorStat(key->String()), GetRMS(key->String()));
    }
  }

  if (fSubResults /* && fSubResults->GetEntries() > 1 */ &&
      (sopt.Contains("ALL") || sopt.Contains("FULL"))) {
...
void AliAnalysisMuMuResult::PrintValue(const char *key, const char *opt,
                                       Double_t value, Double_t errorStat,
                                       Double_t rms) const {
  /// print one value and its associated error

  if (TString(key).Contains("AccEff")) {
    std::cout << opt
              << Form("\t\t%20s %9.2f +- %5.2f %% (%5.2f %%)", key, value * 100,
                      errorStat * 100,
    ...
    std::cout << std::endl;
  } else if (TString(key).BeginsWith("Sigma") ||
             TString(key).BeginsWith("Mean")) {
    std::cout << opt
              << Form("\t\t%20s %9.2f +- %5.2f (%5.2f %%) MeV/c^2", key,
                      value * 1E3, 1E3 * errorStat,
                      value != 0.0 ? errorStat * 100.0 / value : 0.0);
    ...
    std::cout << std::endl;
             ...
             (TString(key).Contains("Fnorm") &&
              !TString(key).Contains("persion"))) {
    std::cout << opt
              << Form("\t\t%20s %9.3f +- %5.3f (%5.2f %%)", key, value,
                      errorStat,
                      value != 0.0 ? errorStat * 100.0 / value : 0.0);
    ...
    if (errorStat > 0.0) {
      std::cout << opt
                << Form("\t\t%20s %9.3f +- %5.3f (%5.3f %%)", key, value,
                        errorStat,
                        value != 0.0 ? errorStat * 100.0 / value : 0.0);
      if (rms) {
        std::cout << Form(" RMS %9.2f (%5.2f %%)", rms, 100.0 * rms / value);
    ...
    if (errorStat > 0.0) {
      ...
      std::cout << opt
                << Form("\t\t%20s %9.2e +- %9.2e (%5.2f %%)", key, value,
                        errorStat,
                        value != 0.0 ? errorStat * 100.0 / value : 0.0);
      if (rms) {
...
//_____________________________________________________________________________
void AliAnalysisMuMuResult::Set(const char *name, Double_t value,
                                Double_t errorStat, Double_t rms) {
  ...
  if (!p) {
    ...
    fKeys = 0x0;
    //    Show("*"); // invalidate as well any printout filter that may have
    //    been set
  } else {
