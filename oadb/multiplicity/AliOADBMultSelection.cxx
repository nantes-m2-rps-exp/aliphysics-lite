...
AliOADBMultSelection::AliOADBMultSelection()
    : TNamed("multSel", ""), fCalibList(0), fCalibListVtxZ(0), fEventCuts(0),
      fSelection(0), fMap(0), fMapVtxZ(0) {
  // constructor
...
AliOADBMultSelection::AliOADBMultSelection(const AliOADBMultSelection &o)
    : TNamed(o), fCalibList(0), fCalibListVtxZ(0), fEventCuts(0), fSelection(0),
      fMap(0), fMapVtxZ(0) {
  fCalibList = new TList();
  fCalibList->SetOwner(kTRUE);
  ...
  while ((obj = next()))
    fCalibList->Add(obj->Clone());
  TIter nextvtxZ(o.fCalibListVtxZ);
  while ((obj = nextvtxZ()))
    fCalibListVtxZ->Add(obj->Clone());
...
AliOADBMultSelection::AliOADBMultSelection(const char *name, const char *title)
    : TNamed(name, title), fCalibList(0), fCalibListVtxZ(0), fEventCuts(0),
      fSelection(0), fMap(0), fMapVtxZ(0) {
  // constructor
  fCalibList = new TList();
  fCalibList->SetOwner(kTRUE);
...
void AliOADBMultSelection::Dissociate() {
  ...
  while ((hist = static_cast<TH1 *>(nextvtxZ())))
    hist->SetDirectory(0);
}
//________________________________________________________________
...
TH1F *AliOADBMultSelection::GetCalibHisto(Long_t iEst) const {
  if (!fCalibList)
    return 0;
  if (iEst < 0 || iEst >= fCalibList->GetEntries())
    return 0;
  return (TH1F *)fCalibList->At(iEst);
}
//________________________________________________________________
TH1F *
AliOADBMultSelection::GetCalibHisto(const TString &lCalibHistoName) const {
  if (!fCalibList)
    return 0;
...
TProfile *AliOADBMultSelection::GetCalibHistoVtx(Long_t iVar) const {
  if (!fCalibListVtxZ)
    return 0;
  if (iVar < 0 || iVar >= fCalibListVtxZ->GetEntries())
    return 0;
  return (TProfile *)fCalibListVtxZ->At(iVar);
}
//________________________________________________________________
TProfile *
AliOADBMultSelection::GetCalibHistoVtx(const TString &lCalibHistoName) const {
  if (!fCalibListVtxZ)
    return 0;
...
void AliOADBMultSelection::Browse(TBrowser *b) {
  ...
  if (b) {
    ...
    cutsFolder->SetOwner();
    estFolder->SetOwner();
    Printf("Browsing operation started on %i objects",
           fCalibList->GetEntries());
    for (Int_t iEst = 0; iEst < fCalibList->GetEntries(); iEst++) {
      if (GetCalibHisto(iEst)) {
        Printf("Adding histo number %i", iEst);
        histoFolder->Add(GetCalibHisto(iEst));
      } else {
        Printf("Histogram numbered %i not found for estimators, skipping",
               iEst);
      }
    }
    if (fCalibListVtxZ) {
      Printf("Browsing operation started on %i objects",
             fCalibListVtxZ->GetEntries());
      for (Int_t iEst = 0; iEst < fCalibListVtxZ->GetEntries(); iEst++) {
        if (GetCalibHistoVtx(iEst)) {
          vertexFolder->Add(GetCalibHistoVtx(iEst));
        } else {
          Printf("Profile numbered %i not found, skipping", iEst);
        }
      }
    }
    ...
    cutsFolder->Add(
        new TObjString(Form("Vertex Z cut: %f", GetEventCuts()->GetVzCut())));
    cutsFolder->Add(new TObjString(
        Form("Physics Selection: %i", GetEventCuts()->GetTriggerCut())));
    cutsFolder->Add(new TObjString(
        Form("INEL > 0: %i", GetEventCuts()->GetINELgtZEROCut())));
    cutsFolder->Add(
        new TObjString(Form("Tracklets Vs Clusters: %i",
                            GetEventCuts()->GetTrackletsVsClustersCut())));
    cutsFolder->Add(
        new TObjString(Form("Reject Pileup SPD (mult bins): %i",
                            GetEventCuts()->GetRejectPileupInMultBinsCut())));
    cutsFolder->Add(
        new TObjString(Form("SPD and Track Vertex Consistency: %i",
                            GetEventCuts()->GetVertexConsistencyCut())));
    cutsFolder->Add(new TObjString(Form(
        "NContribs to PV > 0: %i", GetEventCuts()->GetNonZeroNContribs())));

    const long lNEstimators = fSelection->GetNEstimators();
    TFolder *lEst[lNEstimators];
    for (Int_t iEst = 0; iEst < lNEstimators; iEst++) {
      lEst[iEst] = new TFolder(
          Form("%s_Info", fSelection->GetEstimator(iEst)->GetName()), "Info");
      lEst[iEst]->SetOwner();
      lEst[iEst]->Add(new TObjString(Form(
          "Estimator Name: %s", fSelection->GetEstimator(iEst)->GetName())));
      TString lThisDef = fSelection->GetEstimator(iEst)->GetDefinition();
      lEst[iEst]->Add(
          new TObjString(Form("Estimator Definition: %s", lThisDef.Data())));
      lEst[iEst]->Add(new TObjString(Form(
          "Is an Integer: %i", fSelection->GetEstimator(iEst)->IsInteger())));
      lEst[iEst]->Add(new TObjString(Form(
          "Current Value: %f", fSelection->GetEstimator(iEst)->GetValue())));
      lEst[iEst]->Add(new TObjString(
          Form("Current Percentile: %f",
               fSelection->GetEstimator(iEst)->GetPercentile())));
      lEst[iEst]->Add(new TObjString(
          Form("Mean Value: %f", fSelection->GetEstimator(iEst)->GetMean())));
      lEst[iEst]->Add(new TObjString(Form(
          "Is Anchored: %i", fSelection->GetEstimator(iEst)->GetUseAnchor())));
      lEst[iEst]->Add(new TObjString(
          Form("Anchor Point (raw): %f",
               fSelection->GetEstimator(iEst)->GetAnchorPoint())));
      lEst[iEst]->Add(new TObjString(
          Form("Anchor Percentile: %f",
               fSelection->GetEstimator(iEst)->GetAnchorPercentile())));
